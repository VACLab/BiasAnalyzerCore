...
def condition_cohort_test(bias_obj):
    ...
    if cohort_data:
        ...
        t1 = time.time()
        cohort_concepts = cohort_data.get_concept_stats(concept_type='condition_occurrence')
        # print('the cohort concept condition occurrence stats:')
        # print(pd.DataFrame(cohort_concepts["condition_occurrence"]))
        cohort_de_concepts = cohort_data.get_concept_stats(concept_type='drug_exposure')
