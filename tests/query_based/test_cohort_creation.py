...
def test_cohort_yaml_validation(test_db):
    ...
    invalid_data = {"event_type": "date", "event_concept_id": "dummy"}
...
def test_cohort_creation_baseline(caplog, test_db):
    ...
    assert len(cohort.get_distributions("gender")) == 3, (
        "Cohort get_distribution('gender') does not return 3 gender_bin items"
    )
    ...
    assert_equal(
        patient_106["cohort_start_date"], datetime.date(2023, 3, 1), "Incorrect cohort_start_date for patient 106"
    )
    assert_equal(
        patient_106["cohort_end_date"], datetime.date(2023, 3, 15), "Incorrect cohort_end_date for patient 106"
    )
    assert_equal(
        patient_108["cohort_start_date"], datetime.date(2020, 4, 10), "Incorrect cohort_start_date for patient 108"
    )
    assert_equal(
        patient_108["cohort_end_date"], datetime.date(2020, 4, 27), "Incorrect cohort_end_date for patient 108"
    )
...
def test_cohort_creation_mixed_domains(test_db):
    ...
    cohort = bias.create_cohort(
        ...
        os.path.join(os.path.dirname(__file__), "..", "assets", "cohort_creation", "test_cohort_creation_config.yaml"),
        "test_user",
    )
    ...
    assert_equal(end_dates, [datetime.date(2020, 6, 20), datetime.date(2020, 6, 20), datetime.date(2018, 1, 20)])
...
def test_cohort_invalid(caplog, test_db):
    caplog.clear()
    with caplog.at_level(logging.INFO):
        invalid_cohort = test_db.create_cohort(
            "invalid_cohort", "invalid_cohort", "invalid_yaml_file.yml", "invalid_created_by"
        )
    ...
    with caplog.at_level(logging.INFO):
        invalid_cohort = test_db.create_cohort(
            "invalid_cohort",
            "invalid_cohort",
            os.path.join(os.path.dirname(__file__), "..", "assets", "config", "test_config.yaml"),
            "invalid_created_by",
        )
    ...
    with caplog.at_level(logging.INFO):
        invalid_cohort = test_db.create_cohort(
            "invalid_cohort", "invalid_cohort", "INVALID SQL QUERY STRING", "invalid_created_by"
        )
...
def test_cohort_creation_negative_instance(test_db):
    """
    Test cohort creation with negative event_instance (last occurrence of a condition).
    """
    bias = test_db
    cohort = bias.create_cohort(
        "Diabetes patients (last occurrence)",
        "Cohort of female patients born 1970-2000 with the last Type 2 diabetes diagnosis",
        os.path.join(
            os.path.dirname(__file__), "..", "assets", "cohort_creation", "test_cohort_creation_negative_instance.yaml"
        ),
        "test_user",
    )
    ...
    # Test cohort object and methods
    assert cohort is not None, "Cohort creation failed"
    assert cohort.data is not None, "Cohort creation returned None data"
    ...
    assert_equal(
        ...
        "Incorrect cohort_start_date for patient 1 (last diabetes)",
    )
    assert_equal(
        patient_1["cohort_end_date"],
        datetime.date(2020, 6, 1),
        "Incorrect cohort_end_date for patient 1 (last diabetes)",
    )


def test_cohort_creation_offset(test_db):
    """
    Test cohort creation with non-zero positive and negative offsets.
    """
    bias = test_db
    cohort = bias.create_cohort(
        "Diabetes patients with offset",
        "Cohort of female patients born 1970-2000 with Type 2 diabetes diagnosis, adjusted by +180 and -730 days",
        os.path.join(os.path.dirname(__file__), "..", "assets", "cohort_creation", "test_cohort_creation_offset.yaml"),
        "test_user",
    )
    ...
    assert_equal(
        ...
        "Incorrect cohort_start_date for patient 1 (with -730 day offset)",
    )
    assert_equal(
        patient_1["cohort_end_date"],
        datetime.date(2020, 11, 28),
        "Incorrect cohort_end_date for patient 1 (with +180 day offset)",
    )


def test_cohort_creation_negative_instance_offset(test_db):
    """
    Test cohort creation with negative event_instance and non-zero offset.
    """
    bias = test_db
    cohort = bias.create_cohort(
        "Diabetes patients (last occurrence with offset)",
        "Cohort of female patients born 1970-2000 with the last Type 2 diabetes diagnosis, adjusted by +180 days",
        os.path.join(
            os.path.dirname(__file__),
            "..",
            "assets",
            "cohort_creation",
            "test_cohort_creation_negative_instance_offset.yaml",
        ),
        "test_user",
    )
    ...
    assert_equal(
        patient_1["cohort_end_date"],
        datetime.date(2020, 11, 28),
        "Incorrect cohort_end_date for patient 1 (last diabetes with +180 day offset)",
    )
