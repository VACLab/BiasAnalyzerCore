...
def test_cohort_creation_negative_instance(test_db):
    ...
    cohort = bias.create_cohort(
        ...
        os.path.join(os.path.dirname(__file__), '..', 'assets', 'cohort_creation',
                     'test_cohort_creation_negative_instance.yaml'),
        "test_user"
    )
    ...
    patient_ids = set([item['subject_id'] for item in cohort.data])
    assert_equal(len(patient_ids), 4)  # Female patients 1, 2, 3, 5
    assert_equal(patient_ids, {1, 2, 3, 5})
    ...
    assert_equal(patient_1['cohort_start_date'], datetime.date(2020, 6, 1),
                 "Incorrect cohort_start_date for patient 1 (last diabetes)")
    assert_equal(patient_1['cohort_end_date'], datetime.date(2020, 6, 1),
...
def test_cohort_creation_offset(test_db):
    ...
    cohort = bias.create_cohort(
        ...
        os.path.join(os.path.dirname(__file__), '..', 'assets', 'cohort_creation',
                     'test_cohort_creation_offset.yaml'),
        "test_user"
    )
    ...
    assert_equal(patient_1['cohort_start_date'], datetime.date(2018, 6, 2),
                 "Incorrect cohort_start_date for patient 1 (with -730 day offset)")
    assert_equal(patient_1['cohort_end_date'], datetime.date(2020, 11, 28),
